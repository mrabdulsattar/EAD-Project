...
export const getHotelDisplayImage = (hotel, listIndex = null) => {
  const key = String(hotel?._id || hotel?.name || '0');
  const hashVal = hashString(key);
  const idx = listIndex !== null ? listIndex : hashVal;
  return HOTEL_IMAGE_POOL[Math.abs(idx) % HOTEL_IMAGE_POOL.length];
};

export const getHotelCardImage = (hotel, listIndex = null) =>
  getHotelDisplayImage(hotel, listIndex);

export const getHotelDetailImage = (hotel) => {
  const stored = hotel?.images?.[0] || hotel?.image;
  if (stored && String(stored).startsWith('http')) return stored;
  return getHotelDisplayImage(hotel);
};

export const handleHotelImageError = (e, hotel, listIndex = null) => {
  e.currentTarget.onerror = null;
  e.currentTarget.src = getHotelDisplayImage(hotel, listIndex);
};